...
$.fuckUAddAddress='{{#this}}\
<div class="am-container addAddressPage">\
  <form action="../doaction.php" data-typeEdit="{{typeEdit}}" data-adr-id="{{adr-id}}" data-address="{{address}}" method="post" name="addform" id="addressForm" class="am-form" data-am-validator>\
  <div class="am-form-group">\
    <label for="">收件人姓名</label>\
    <input type="text" name="truename" class="" id="truename" placeholder="输入收件人姓名" value="{{name}}" required>\
  </div>\
  <div class="am-form-group">\
    <label for="">手机号/联系电话</label>\
    <input type="text" name="phone" class="js-pattern-myphone" id="phone" placeholder="输入手机号/联系电话" value="{{phone}}" required>\
  </div>\
  \
  <div class="am-form-group">\
  ...
  <div class="am-form-group">\
    <label for="">地址</label>\
    <input type="text" name="pre-address" class="" id="address" placeholder="填写详细地址" required>\
  </div>\
...
$.fuckUPassword='{{#this}}\
<div class="am-container passwordPage">\
  <form action="../doaction.php" method="post" name="userinfoform" id="userinfoform" class="am-form" data-am-validator>\
  ...
  <div class="am-form-group">\
    <label for="">原密码</label>\
    <input type="password" name="oldpassword" class="" id="" placeholder="输入原密码" value="" required>\
  </div>\
  \
  <div class="am-form-group">\
    <label for="province">新密码</label>\
    <input type="password" name="password" class="" id="newPassword" minlength="6" maxlength="12" placeholder="输入6-12位新密码" value="" required>\
  </div>\
  \
  <div class="am-form-group">\
    <label for="city">确认新密码</label>\
    <input type="password" name="repassword" class="" id="rePassword" placeholder="确认新密码" value="" data-equal-to="#newPassword" required>\
  </div>\
  <button class="am-btn am-btn-primary am-btn-block" type="submit" id="gotoSubmit">确认修改</button>\
</form>\
</div>\
{{/this}}';
  ...
  <div class="am-tabs-bd am-g">\
    ...
    <div class=" am-tab-panel payItem" id="tab{{@index}}">\
      ...
      <p class="payItemDsc">{{{payItemDsc}}}</p>\
        ...
        <div class="supportBox am-u-sm-12 am-cf"><p class="progressGet am-fr">提交订单前请仔细阅读回报内容</p></div>\
    </div>\
    {{/if}}\
    {{/each}}\
...
<div class="selectPay fixHide am-container" id="selectPay">\
  ...
  <div class="am-u-sm-12 am-form-group selectPayBox">\
    ...
    <p style="font-size: 12px;color: #999;">支付提示：微信用户请使用微信支付</p>\
  </div>\
  <form action="/e/ShopSys/doaction.php" method="post" name="addform" id="finalForm" class="am-form">\
    ...
    <input type="submit" id="finalFormSubmit" class="am-btn am-btn-success am-btn-block" value="去支付"/>\
  </form>\
  <div class="am-modal am-modal-loading am-modal-no-btn" tabindex="-1" id="waitingToPay">\
    <div class="am-modal-dialog">\
      <div class="am-modal-hd">正在跳转支付页面，请稍候</div>\
